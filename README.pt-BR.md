...
Site estático e local da **Alpha 0.1.0**, baseado no template `repository-site` fornecido. A página principal é em inglês (`index.html`), com uma versão completa em português brasileiro (`pt-br.html`). Não inclui workflow de implantação. O arquivo `CNAME` registra o domínio previsto, `gravewright.com`.
