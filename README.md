...
SEO metadata is generated by `scripts/seo.mjs`: localized titles and descriptions, canonical URLs, language alternates, Open Graph, large-image social cards and WebSite/WebPage/SoftwareApplication JSON-LD. The video cover is also the sharing image. Update `content/media.json` when changing it; the checks verify the image dimensions. `sitemap.xml` lists both public pages and `robots.txt` points crawlers to it.
